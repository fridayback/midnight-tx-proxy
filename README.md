...
```ini
# 钱包种子（二选一：使用 SEED 或 KEYSTORE_PATH 方式）
...
| 方式 | 说明 |
|------|------|
| **SEED 环境变量** | 直接设置钱包种子 hex 字符串，简单直接 |
| **以太坊 Keystore 文件** | 从以太坊 UTC/JSON keystore 文件解密私钥作为 seed，更安全 |

**Keystore 方式额外配置：**

```ini
# Keystore 文件路径（使用 keystore 方式时必填）
# KEYSTORE_PATH=./keystore.json

# Keystore 解密密码（使用 keystore 方式时必填）
# KEYSTORE_PASSWORD=yourpassword
```

密码优先级：`--keystore-password` 命令行参数 > `KEYSTORE_PASSWORD` 环境变量
...
**方式二：使用以太坊 Keystore 文件**

```bash
# 通过命令行参数指定 keystore 路径和密码
tsx src/index.ts --keystore ./keystore.json --keystore-password yourpassword

# 通过环境变量指定密码
KEYSTORE_PASSWORD=yourpassword tsx src/index.ts --keystore ./keystore.json

# 路径和密码都通过环境变量指定
KEYSTORE_PATH=./keystore.json KEYSTORE_PASSWORD=yourpassword tsx src/index.ts

# 生产模式（构建后运行）
npm run build
node dist/index.js --keystore ./keystore.json --keystore-password yourpassword
```
...
如需使用以太坊 keystore 文件替代 SEED 环境变量，将 keystore JSON 文件放在项目根目录：

```bash
# 将 keystore 文件放到项目根目录
cp /path/to/your/keystore.json ./keystore.json
```

然后在 `config/.env` 中添加密码（路径由 docker-compose.yml 固定为 `/app/keystore.json`）：
...
> `KEYSTORE_PATH` 已在 docker-compose.yml 中预设为 `/app/keystore.json`，无需额外配置。
